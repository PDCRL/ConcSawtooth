...
def generate_word_list(count):
    ...
    f = open("file","r")
    return {x.strip(): None for x in f.readlines()[0:count]}
...
def do_generate(args, batches, keys):
    ...
    signer = crypto_factory.new_signer(private_key)
    length = len(keys)
    v=list(keys.keys())[:length]	#creating keys list
    print(v)
    start = time.time()
    total_txn_count = 0
    j = 0
    ...
    for i in lines.split():
	       indexes.append(int(i))	#creating index list input
    #print(v[indexes[0]]) 
    #print(v[indexes[1]]) 
    counts = 0
    for i in range(0, args.count):
        txns = []
        for _ in range(0, args.max_batch_size):
            name = v[indexes[counts]]
            counts+= 1
            txn = create_intkey_transaction(
                verb=random.choice(['inc', 'dec']),
                name=name,
		#value=int(fl.readlines()[v].strip()),
...
def write_batch_file(args, batches):
    ...
    print("Hello Writing to {}...".format(args.output))
